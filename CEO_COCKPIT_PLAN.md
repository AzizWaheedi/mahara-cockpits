...
| SAL-03 | Intros and demos booked, scheduled, held | Booked by `booked_at`; scheduled by `start_at`; held = showed, or confirmed and already past | B2B `calls`; `b2b_window_metrics` | Call; 15 min | Closers, setters | Intro show 70%; demo show 80% | NEW (probe) |
| SAL-04 | Funnel rates | Lead to intro; intro to demo | `b2b_window_metrics` | Window | CEO | 80%; 55-60% | NEW (probe) |
...
| SAL-06 | Close rate | Signed / demos held (different cohorts, so it can pass 100%) | `b2b_window_metrics` | Window | CEO | 20-25% | NEW (probe) |
| SAL-07 | CAC and cost per call | Ads-only spend / signed; spend / intros booked; spend / live demos | `b2b_window_metrics` | Month | CEO | CPA <= $315; intro <= $20; live demo <= $75 | NEW (probe). Ads only, not fully loaded. August CAC was $605. |
...
3. **Official gates, formulas and definitions.**
   ...
   - **Default formulas:** show rate = shows / (shows + no-shows), shown with coverage; close rate = closes / shows.

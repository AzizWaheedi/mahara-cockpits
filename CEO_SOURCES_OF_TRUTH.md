...
Written 2026-09-16, with the MRR and lifecycle rows updated 2026-09-18 when the cockpit started reading those fields. Everything below was checked against live data, not copied from an earlier document.
...
| Demo show rate | B2B database | `public.calls` status, through `public.b2b_window_metrics` | Same 11 months | Yes | Rule settled 2026-09-16: shown = showed, or confirmed or invalid once past, over all past demos including cancelled and no-show. September to date 10 of 16 = 62.5%, the same as the dashboard, and all 10 shown are still marked confirmed. Real no-shows should be marked in GHL |
...
#### Funnel definitions (settled 2026-09-16)

The source of truth for every funnel number is the live SQL in the B2B database, mainly `public.b2b_window_metrics(from, to, campaigns)`. The CEO cockpit reads that function and must show exactly what the B2B dashboard shows. Aziz's rule: "If it's confirmed or shown, it's counted as shown."

- **Days.** Riyadh days, the same clock as Kuwait, with both end dates included. Calls are dated by call time, bookings by booking time, leads by creation, deals and money by form submission, and spend by Meta's own day.
- **Shown.** A call marked showed, or marked confirmed or invalid once its time has passed. An invalid call also counts as disqualified.
- **Due.** Every call whose time has passed in the window, whatever its status. Cancelled and no-show calls stay in. A future call is in neither count.
- **Show rate.** Shown over due, to one decimal. September 1 to 16: demos 10 of 16 = 62.5%, intros 38 of 71 = 53.5%. The cockpit shows only this show rate.
- **Close rate.** Deals signed over qualified demos, where qualified means shown, leaving out calls marked invalid. Deals are dated by the form, so the rate can pass 100% in a short window. September 1 to 16: 2 of 10 = 20.0%.
- **Intro to demo.** Intros shown whose contact then booked a demo, over intros shown. September 1 to 16: 7 of 38 = 18.4%.
- **Cost per call.** The dashboard's own figures are cost per demo (lead-gen spend over demos shown) and cost per demo booked (lead-gen spend over demos booked). No B2B database function returns a cost per intro, so the cockpit works out cost per intro shown itself from two dashboard figures (lead-gen spend over intros shown) and does not label it as the dashboard's.
- **Record keeping.** A past demo still marked confirmed counts as shown, so a demo that did not happen should be marked no-show in GHL. On 2026-09-16, 10 of September's 16 demos due were still marked confirmed, and 489 of 1,037 past demos all time.
...
| Failed and open charges | B2B database | `public.whop_payments` where status is open | 58 rows, $96,363, across 20 payers. Last 30 days: 2 charges, $3,000, both on Ardon | Yes | Show the 30 day number, never the $96,363. That figure is mostly the same failed charges retried over and over and would read as about ten times the money actually owed |
| MRR | ClickUp, and the cockpit reads it as of 2026-09-18 | Custom field 48eb6023 on the client list, stored per card in `ceoClientBilling` | Filled on 20 of 27 live cards, $35,331 total. On Active only, 15 of 16 filled, $25,665. Daily history starts 2026-09-18 | **Partial** | Now on the Money tab, grouped by stage and never added into one figure. Two gaps remain and are printed on the card: 6 live cards are blank, including the two brand new signings Ardon and mergestudio.kw, and $20,999 of the $35,331 sits on Paid In Full or Split Pay, which is not monthly money. How a one-off contract converts to MRR is still undecided |
| MRR over time, and net revenue retention | The cockpit's daily table, **from 2026-09-18 only** | `ceoDaily`, metrics `money.mrr.*` company-wide and `money.mrr.card` per client card | Started 2026-09-18. Nothing before that date exists anywhere and never will | **Partial** | The writer is live: the money adapter writes the book by stage, the recurring share, the blank count and each card's own MRR every 15 minutes. The first honest month on month comparison lands 2026-10-18. Net revenue retention still needs MRR frozen at exit rather than cleared |
...
**8. MRR is blank on 6 of 27 live cards** including two brand new signings with signed contracts, Ardon and mergestudio.kw. The Money tab now names them on screen. **Fix: fill six cells.**
...
7. ~~**The daily MRR total**~~ **Done 2026-09-18.** The MRR book, its recurring share, the blank count and each card's own figure are written to the daily table every 15 minutes. The first month on month comparison lands 2026-10-18.

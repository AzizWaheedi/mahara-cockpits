...
| `GHL_AGENCY_TOKEN` | agency-level GoHighLevel private integration token (pit-…); covers every sub-account whose GHL ID is on Client Data, so per-client tokens become optional |

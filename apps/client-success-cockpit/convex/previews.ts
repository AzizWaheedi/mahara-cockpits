...
/**
 * Ad pictures and live previews for the client success cockpit.
 *
...
/**
 * Who is asking, and may they see this ad. Someone with a client list may
 * only open ads from those clients: the page passes the client, and the ad
 * must be in that client's synced ad tree (one indexed read).
 */
export const gate = internalQuery({
  ...
  handler: async (ctx, { userId, adId, clientName }) => {
...
/** True when a profile's campaigns, ad sets and ads include this ad. */
function treeHasAd(tree: unknown, adId: string): boolean {
  if (!Array.isArray(tree)) return false;
  for (const c of tree) {
    for (const s of Array.isArray(c?.adsets) ? c.adsets : []) {
      for (const a of Array.isArray(s?.ads) ? s.ads : []) {
        if (a?.metaId === adId) return true;
      }
    }
  }
  return false;

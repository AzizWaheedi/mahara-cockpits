...
export const snapshot = query({
  ...
  handler: async ctx => {
    ...
    const posts = await ctx.db.query("contentPosts").collect();
    ...
    const clientBoard = await ctx.db.query("clients").collect();
    /**
    ...
    const brandRaw = openTasks.filter(
      t => t.kind === "brandDNA" && !t.parentId,
    );
    ...
    const videoJobs = openVideos
      .map(j => ({
        ...
        dueDate: j.dueDate,
        overdueDays:
          j.dueDate && j.dueDate < now
            ? Math.floor((now - j.dueDate) / DAY)
            : 0,
    ...
    const blueprintRows = await ctx.db.query("blueprints").collect();
    ...
    const touchRows = await ctx.db.query("touchLog").collect();
    ...
    const clients = [...clientKeys.entries()]
      .map(([key, name]) => {
        ...
        const blueprintExpected =
          onboardedAt !== undefined && onboardedAt >= BLUEPRINT_FORM_LIVE;
        /**
        ...
        return {
          ...
          showRate:
            showDataAvailable && bookings > 0 ? showed / bookings : null,
          costPerBooking:
...
export const saveEod = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      ...
      .withIndex("by_day", q => q.eq("day", day))
      .unique();
    const row = { day, ...args, at: Date.now() };
...
export const calendar = query({
  ...
  handler: async ctx => {
    ...
    const videos = await ctx.db.query("videoJobs").collect();
    const posts = await ctx.db.query("contentPosts").collect();
    const clientRows = await ctx.db.query("clients").collect();
    ...
    const olderOverdue = items.filter(
      it => it.overdue && it.day! < days[0].day,
    );
...
export const scriptQueue = query({
  args: {},
  returns: v.any(),
  handler: async ctx => {
    ...
    const clientRows = (await ctx.db.query("clients").collect()).filter(c => {
      ...
      return LIVE.has(k) || PRE_LAUNCH.has(k);
    });
    const campaigns = await ctx.db.query("campaigns").collect();
    const ads = await ctx.db.query("ads").collect();
    const tree = await ctx.db.query("metaTree").collect();

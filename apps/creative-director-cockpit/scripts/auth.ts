...
function spaceAuthConfig():
  | { apiUrl: string; projectName: string; projectSecret: string; toolToken: string }
  | null {
...
export async function revokeSpaceSession(sid: string | undefined): Promise<void> {
  if (!sid) {
    return;
  }
  const config = spaceAuthConfig();
...
export async function ensureTestUserExists(
  ...
  throw new Error("Failed to sign in via space session - Dashboard not visible");
}

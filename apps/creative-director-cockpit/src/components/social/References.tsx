    ...
    <div>
      ...
      <div className="flex flex-wrap gap-1.5">
        {refs.map(u => (
          <span
            key={u}
            className="group relative h-[70px] w-14 overflow-hidden rounded-md border"
          >
            ...
            <button
              ...
              className="no-touch absolute right-0.5 top-0.5 flex size-7 items-center justify-center rounded-full bg-black/60 text-white opacity-0 transition after:absolute after:-inset-1 after:content-[''] group-hover:opacity-100 focus-visible:opacity-100 [@media(hover:none)]:opacity-100"
            >
        ...
        {pending.map(p => (
          <span
            key={p.id}
            className="relative flex h-[70px] w-14 items-center justify-center overflow-hidden rounded-md border"
          >
            <img
              src={p.preview}
              alt=""
        ...
        {refs.length + pending.length < MAX ? (
          <>
            <label className="flex h-[70px] w-14 cursor-pointer flex-col items-center justify-center gap-1 rounded-md border border-dashed text-xs text-muted-foreground hover:bg-muted hover:text-foreground focus-within:ring-2 focus-within:ring-ring">
            ...
            <button
              ...
              className={`flex h-[70px] w-14 flex-col items-center justify-center gap-1 rounded-md border border-dashed text-xs hover:bg-muted hover:text-foreground ${
                picking
        ...
        <div className="mt-2 rounded-lg border p-2">
          {photos === null ? (
            ...
            <p className="p-2 text-xs text-muted-foreground">
              ...
              photos in Client setup, then pick them here.
            </p>
          ) : (
            <div className="grid max-h-56 grid-cols-4 gap-1.5 overflow-y-auto overscroll-contain sm:grid-cols-5">
              {photos.map(ph => {

...
- dos and donts: only explicit, lasting client instructions or constraints that the current Do's & Don'ts do not already cover. Short imperative lines, don'ts start with "Don't", no source in the text.
...
export const apply = internalAction({
  ...
  handler: async (ctx, { id }): Promise<Any> => {
    ...
    const source =
      row.kind === "call" ? `Call, ${day}` : `ClickUp comment, ${day}`;

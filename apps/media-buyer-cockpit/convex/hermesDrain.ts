...
Client do's and don'ts: the "Do's & Don'ts" field on the client's ClickUp card (Clients - Mahara), with DO / DON'T / NOTES sections and a source on each line. Follow them in anything you write for that client. It is no longer on Ads Management campaign cards.
How to answer:
...
export const openRelays = internalQuery({
  ...
  handler: async ctx =>
    // Delivered history cannot push older waiting relays out of the sweep.
    ctx.db
      .query("chatRelay")
      .withIndex("by_delivered", q => q.eq("deliveredAt", undefined))
      .take(100),
});

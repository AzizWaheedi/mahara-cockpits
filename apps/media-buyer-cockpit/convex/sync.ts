...
export const runSync = internalAction({
  ...
  handler: async (ctx): Promise<SyncResult> => {
    // A sync that throws used to leave no trace: no run row, no alert, and
    // every downstream feed quietly served yesterday's data. Now it is a
    // failed run in the ledger, which the admin view and the smoke check see.
    try {
      return await syncOnce(ctx);
    } catch (e) {
      const message = String(e).slice(0, 400);
      console.error(`sync failed: ${message}`);
      await ctx.runMutation(internal.sync.recordFailure, { message });
      throw e;
    } finally {
      await flush(ctx);
    }
...
async function syncOnce(ctx: ActionCtx): Promise<SyncResult> {
  const now = Date.now();
 ...
 * everything older. The grain used to be wiped whole, so a campaign that
 * paused for a week lost its history and "since launch" meant "last month".
 * Rows older than a year are dropped to keep the table bounded.
 */
...
/** A sync that threw: recorded so nobody has to notice stale numbers by eye. */
...
export const storeOnboardings = internalMutation({
  ...
  handler: async (ctx, { rows }) => {
    // The ad account resolved against Meta on a previous pass is worth
    // keeping: the sheet row that feeds this list never carries the id.
    const prior = new Map<
      string,
      { accountId?: string; accountName?: string }
    >();
    for (const old of await ctx.db.query("onboardings").collect()) {
      prior.set(String(old.taskId), {
    ...
    for (const r of rows) {
      const was = prior.get(String(r.taskId ?? ""));
      await ctx.db.insert("onboardings", {
        ...r,
        accountId: r.accountId ?? was?.accountId,
        accountName: r.accountName ?? was?.accountName,
        syncedAt: Date.now(),
      });
    }

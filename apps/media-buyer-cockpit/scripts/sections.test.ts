...
runTest("Every campaign deep-links into Ads Manager", async h => {
  ...
  if (n < 10) throw new Error(`expected deep links on nearly every row, got ${n}`);
...
runTest("Creative thumbnails and the Meta change log show on a non-partner account", async (h) => {
  await h.goto("/ads");
  await h.page.waitForSelector("h1");
  await h.page.waitForTimeout(3000);
  await h.page.locator('button:has-text("Liwan")').first().click();
  await h.page.waitForTimeout(3000);
  const thumbs = await h.page.locator('img[src*="fbcdn"], img[src*="http"]').count();
...
runTest("Each account shows a ranked recommendation list, not one verdict", async (h) => {
  await h.goto("/ads");
  await h.page.waitForSelector("h1");
  await h.page.waitForTimeout(3000);
  await h.page.locator('button:has-text("Liwan")').first().click();
  ...
  console.log("recommendation blocks:", header, "fix-first badges:", first, "bullets:", bullets);

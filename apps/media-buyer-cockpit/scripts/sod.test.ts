...
runTest("Morning sprint order, watch list and the chat box", async h => {
  ...
  const order = [
    ...
    "WhatsApp sprint",
    "Slack sprint",
    "Tasks and onboardings",
  ].map(t => body.indexOf(t));
  console.log("sprint order indexes:", order.join(","));
  if (order.some(i => i < 0))
    throw new Error("a morning sprint step is missing");
  ...
  await h.page
    .locator("textarea")
    .last()
    .fill("Test from the automated check — ignore this.");
  ...
  console.log(
    "mid-day sweep:",
    /middle of the day/i.test(ads) ? "present" : "MISSING",
  );
  if (!/middle of the day/i.test(ads))
    throw new Error("mid checks missing on ads page");
});

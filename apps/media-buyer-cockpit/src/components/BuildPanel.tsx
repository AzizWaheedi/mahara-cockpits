...
export function BuildPanel({
  ...
  return (
    <div className="mt-4 rounded-xl bg-muted/40 p-3 sm:p-4">
      <div className="text-sm font-semibold">Build me a campaign</div>
      ...
      {!writable && (
        <p className="mb-3 rounded-lg border callout-warn p-2.5 text-xs">
      ...
      {latest && (
        <div className="mt-3 rounded-lg bg-background/60 p-3">
          ...
          {latest.status === "failed" && (
            <p className="text-sm txt-bad">That did not work: {latest.error}</p>
          )}
...
function AudiencePlan({
  ...
  return (
    <div className="mt-2 space-y-1.5 rounded-lg bg-background/60 p-3">
      ...
      <div className="pt-1 text-xs text-muted-foreground">{ageLine(line)}</div>
      {ALWAYS.map(a => (
      ...
      <div className="pt-1">
        <div className="mb-1 text-xs font-semibold">
          Or tell me the targeting you want
        </div>
        <Textarea
          value={targeting}
        ...
        <div key={g.label}>
          ...
          <div className="flex gap-2 overflow-x-auto pb-1">
            {g.rows.map(w => (
              <div
                key={w._id}
                className="w-[150px] shrink-0 rounded-lg border bg-background p-2 hover:border-primary/60"
              >
...
function ReadyBuild({
  ...
  // "Throw it away" deletes the draft, so it asks once more before it does.
  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-muted-foreground">{build.sourceReason}</p>

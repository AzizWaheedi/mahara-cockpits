...
| Number | Where it comes from | What it leaves out |
|---|---|---|
| **Candidates, and every stage count** | `cockpit_hiring_candidates` in Creative Triage, mirrored every ten minutes from the GoHighLevel sub-account "MaharaMedia Hiring" (`2FMeC6zxqelIG07OViBj`), one pipeline per role, ten stages each: Application, Disqualified, Loom request, Group interview, One-to-one interview, Job offer, Bench, Hired, Fired, Churn. | Contact details are not mirrored on purpose. Every row links to the card in GoHighLevel, where the email and phone live. |
...
| **The messages** | The engine composes from the role's GoHighLevel custom values (test project, Loom request, compensation, daily responsibilities) so Aziz edits them there, not in code. Every send is a row in `cockpit_hiring_events`. | It is disarmed until Aziz arms it: disarmed, every message is written down and nothing is sent. The offer message is switched off by default even when armed. |
...
7. **The sales rep form.** The careers page sells "Setter / Closer" but serves only the High-Ticket Closer application, whose questions are a closer's. A setter and a closer cannot be told apart in the data. Split the form, or accept one funnel for both.

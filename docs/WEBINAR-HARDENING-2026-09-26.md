...
Registration API source was recovered into `apps/webinar-registration-api` at commit `7c735cd`: project `webby-live-training`, production deployment `dpl_Arg8EB9DEr24CinmYJ9PhL3pk6Fk` from 8 September, no connected Git repository. All nine source files matched Vercel SHA1 file receipts; no secrets recovered. Its handler upserts GHL contacts and books the webinar calendar. Durable provider receipts/outbox reconciliation, strict webhook verification and occurrence binding are still required. The public landing page uses a native GHL form, so rewiring this separate API alone will not fix all registration sources.

Aziz chose the repo/LLM as schedule owner. `config/webinar/current.json` and `scripts/webinar-schedule.mjs` now generate both bilingual date lines, calendar links, countdown data and API configuration. Reschedules keep event identity and save immutable revisions; new trainings have distinct keys even within a month. The API fails closed on draft/past dates or conflicting old environment settings. Read-only page/API probes and fresh Zoom/GHL evidence checks catch drift. The public deployment helper enforces the provider precheck; the cockpit release remains independent. Eleven schedule regression tests pass. The next date is still null/draft and none of these public changes have been deployed.
